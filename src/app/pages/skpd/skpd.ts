    ...
    deleteItem(item: Skpd): void {
        this.confirmationService.confirm({
            ...
            accept: () => {
                ...
                this.messageService.add({ severity: 'success', summary: 'Berhasil', detail: 'SKPD berhasil dihapus' });
            },
        });
    }
    ...
    saveItem(): void {
        ...
        if (this.isEditMode()) {
            this.data.update((list) =>
                list.map((d) => (d.id === current.id ? (current as Skpd) : d))
            );
            this.messageService.add({ severity: 'success', summary: 'Berhasil', detail: 'SKPD berhasil diperbarui' });
        } else {
            const newId = Math.max(...this.data().map((d) => d.id), 0) + 1;
            this.data.update((list) => [...list, { id: newId, nama: current.nama! }]);
            this.messageService.add({ severity: 'success', summary: 'Berhasil', detail: 'SKPD berhasil ditambahkan' });
        }
